...
readonly KUBE_GO_PACKAGE=gitlab.aibee.cn/platform/ai-scheduler
...
#  gitlab.aibee.cn/platform/ai-scheduler/cmd/kube-apiserver
#  gitlab.aibee.cn/platform/ai-scheduler/cmd/kube-controller-manager
#  gitlab.aibee.cn/platform/ai-scheduler/cmd/kube-scheduler
  gitlab.aibee.cn/platform/ai-scheduler/cmd/ai-scheduler
#  gitlab.aibee.cn/platform/ai-scheduler/cmd/kube-proxy
#  gitlab.aibee.cn/platform/ai-scheduler/cmd/kubelet
)
...
# Argument: the name of a Kubernetes package (e.g. gitlab.aibee.cn/platform/ai-scheduler/cmd/kube-scheduler)
...
import (
  "testing"
  "gitlab.aibee.cn/platform/ai-scheduler/pkg/util/coverage"
)
    ...
    for package in "$@"; do
      if kube::golang::is_instrumented_package "${package}"; then
        ...
        go test -c -o "$(kube::golang::outfile_for_binary "${package}" "${platform}")" \
          -covermode count \
          -coverpkg gitlab.aibee.cn/platform/...,gitlab.aibee.cn/platform/ai-scheduler/vender/k8s.io/... \
          "${build_args[@]}" \
          -tags coverage \
          "${package}"
      else
